...
import com.mnw.mapper.MlpBqsMapper4;
 ...
 */
public class MlpBqsMr extends Configured implements Tool {

    private static final Logger LOGGER = LoggerFactory.getLogger(MlpBqsMr.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            int status = ToolRunner.run(configuration, new MlpBqsMr(), args);
    ...
    public int run(String[] strings) throws Exception {
        ...
        // 2.Create Job
        Job job1 = Job.getInstance(conf, "GetBqsData1");
        ...
        conf.set("mapreduce.map.java.opts", "-Xmx3152m");
        Job job2 = Job.getInstance(conf, "joinBqsData2");
        job2.setJarByClass(MlpBqsMr.class);
...
//        job1.setNumReduceTasks(11);
        ...
        Job job3 = Job.getInstance(conf, "joinBqsData3");
        job3.addCacheFile(cacheFile);
        job3.setJarByClass(MlpBqsMr.class);
//        job1.setNumReduceTasks(11);
        ...
        job4.addCacheFile(inPath6);
        job4.setJarByClass(MlpBqsMr.class);
        FileInputFormat.addInputPath(job4, new Path(conf.get("outPath3")));

        job4.setMapperClass(MlpBqsMapper4.class);
        job4.setMapOutputKeyClass(Text.class);
        job4.setMapOutputValueClass(MapWritable.class);

        job4.setReducerClass(MlpBqsReduce4.class);
